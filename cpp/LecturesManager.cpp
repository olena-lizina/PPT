...
LecturesManager::LecturesManager(QObject* parent)
    : QObject(parent)
{
    auto disciplines = mSaveManager->loadTeachDiscipline();
    auto chapters = mSaveManager->loadChapters();
    auto themes = mSaveManager->loadTheme();
    auto subtheme = mSaveManager->loadSubtheme();

    QList<QList<QPair<int, QString> > > data;
    QList<QPair<int, QString> > tmp;

    for (auto d : disciplines)
        tmp << QPair<int, QString> (d.id, d.name);

    data << tmp;
    tmp.clear();

    for (auto c : chapters)
        tmp << QPair<int, QString> (c.id, c.name);

    data << tmp;
    tmp.clear();

    for (auto t : themes)
        tmp << QPair<int, QString> (t.id, t.name);
